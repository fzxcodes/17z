...
async function startHisoka() {
    ...
    hisoka.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`Bad Session, Mohon koneksikan ulang/scan.`); hisoka.logout(); }
            else if (reason === DisconnectReason.connectionClosed) { console.log("Koneksi terputus, menghubungkan ulang..."); startHisoka(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Koneksi terputus dari server, menghubungkan ulang..."); startHisoka(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Koneksi di replace oleh sesi lain, tutup dulu sesi ini."); hisoka.logout(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Device keluar, scan ulang.`); hisoka.logout(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("Restart dibutuhkan, merestart..."); startHisoka(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Waktu koneksi habis karna ada masalah/sinyal nya lagi jelek, Mengoneksi ulang.."); startHisoka(); }
            else hisoka.end(`Unknown DisconnectReason: ${reason}|${connection}`)
        }
        console.log('Terkoneksi.', update)
    })
